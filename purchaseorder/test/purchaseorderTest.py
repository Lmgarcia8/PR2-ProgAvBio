...
class PurchaseOrderTestMethods(unittest.TestCase):
    ...
    def test_should_buy_remove_the_product_if_the_amount_is_enough(self):
        store: Store = mock()
        when(store).there_are_enough_products("milk", 25).thenReturn(True)
        purchase_order = PurchaseOrder("milk", 25)
        purchase_order.buy(store)

        verify(store, times=1).remove_products("milk", 25)
